---
description: Integrasi Supabase sebagai alternative backend dengan GetX state management: Authentication, PostgreSQL Database, Rea...
---
# Workflow: Supabase Integration (Auth + PostgreSQL + Realtime + Storage)

// turbo-all

## Overview

Integrasi Supabase sebagai alternative backend dengan GetX:
Authentication, PostgreSQL Database, Realtime subscriptions,
dan Storage.
...
- Supabase project created di supabase.com


## Agent Behavior

- **Jangan hardcode Supabase URL/key** — gunakan env configuration.
- **Selalu setup RLS** — Row Level Security di setiap tabel.
- **Gunakan realtime** untuk live updates.
...
class SupabaseAuthRepository implements AuthRepository {
  ...
  Future<Result< User>> signInWithEmailAndPassword(
    ...
    try {
      ...
      if (response.user == null) {
        return const Err(AuthFailure('Sign in failed'));
      }
  ...
  Future<Result< User>> signInWithOAuth(String provider) async {
    try {
      ...
      if (!response) {
        return const Err(AuthFailure('OAuth sign in failed'));
      }
      ...
      return const Err(AuthFailure('OAuth sign in timeout'));
    } on AuthException catch (e) {
      return Err(AuthFailure(e.message));
    } catch (e) {
      return Err(AuthFailure(e.toString()));
  ...
  Future<Result< String>> uploadImage({
    ...
    try {
      ...
      if (compressedBytes == null) {
        return const Err(StorageFailure('Gagal compress gambar'));
      }
