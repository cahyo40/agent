---
description: Comprehensive testing (unit, widget, integration) dan production preparation untuk Flutter app dengan GetX state mana...
---
# Workflow: Testing & Production (Unit + Widget + CI/CD)

// turbo-all

## Overview

Comprehensive testing (unit, widget, integration) dan production
preparation untuk Flutter app dengan GetX. Termasuk CI/CD
pipeline dan deployment setup.
...
- **Coverage target ≥ 80%** untuk unit tests.
- **Test semua GetxController** dengan Get.put() / Get.lazyPut().
- **Mock dependencies** dengan mocktail.
...
void main() {
  ...
  group('ProductController - Initialization', () {
    ...
    test('should handle error on fetch products failure', () async {
      when(() => mockRepository.getProducts())
          .thenAnswer((_) async => Err(ServerFailure('Server error')));
  ...
  group('ProductController - CRUD Operations', () {
    test('should add product successfully', () async {
      ...
      when(() => mockRepository.addProduct(any()))
          .thenAnswer((_) async => Success(newProduct));
      when(() => mockRepository.getProducts())
          .thenAnswer((_) async => Success([newProduct]));
    ...
    test('should update product successfully', () async {
      ...
      when(() => mockRepository.updateProduct(any()))
          .thenAnswer((_) async => Success(updatedProduct));
      when(() => mockRepository.getProducts())
          .thenAnswer((_) async => Success([updatedProduct]));
    ...
    test('should handle add product failure', () async {
      ...
      when(() => mockRepository.addProduct(any()))
          .thenAnswer((_) async => Err(ServerFailure('Gagal menambah produk')));
  ...
  group('ProductController - Search & Filter', () {
    ...
    setUp(() {
      when(() => mockRepository.getProducts())
          .thenAnswer((_) async => Success(allProducts));
    });
  ...
  group('AuthService - Login', () {
    test('should login successfully and save token', () async {
      ...
      when(() => mockAuthRepo.login(email, password))
          .thenAnswer((_) async => Success(AuthResult(user: user, token: token)));
    ...
    test('should handle login failure', () async {
      when(() => mockAuthRepo.login(any(), any()))
          .thenAnswer((_) async => Err(AuthFailure('Invalid credentials')));
  ...
  group('AuthService - Token Refresh', () {
    test('should refresh token when expired', () async {
      ...
      when(() => mockAuthRepo.refreshToken(oldToken))
          .thenAnswer((_) async => Success(newToken));
