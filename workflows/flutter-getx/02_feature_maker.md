---
description: Generate feature baru dengan struktur Clean Architecture lengkap menggunakan GetX pattern.
---
# Workflow: Feature Maker (GetX Clean Architecture Generator)

// turbo-all

## Overview

Generator untuk membuat feature baru dengan struktur Clean
Architecture lengkap menggunakan GetX pattern: domain, data,
presentation layers.
...
- GetX dan dependencies ter-install


## Agent Behavior

- **Tanya nama feature** — contoh: `product`, `order`, `user`.
- **Tanya fields** — list fields yang dibutuhkan entity.
- **Auto-generate semua layers** — domain, data, presentation.
- **Auto-register routes** — tambahkan ke GetX routing.
...
- Dependencies sudah terinstall (`get`, `get_storage`, `dio`, `equatable`, `shimmer`)
...
2. Repository abstract contract (menggunakan `Result<T>` sealed class)
...
abstract class {FeatureName}Repository {
  ...
  Future<Result< {FeatureName}>> create{FeatureName}({FeatureName} {featureName});
  Future<Result< {FeatureName}>> update{FeatureName}({FeatureName} {featureName});
  Future<Result< Unit>> delete{FeatureName}(String id);
}
...
class Get{FeatureName}s implements UseCase<List<{FeatureName}>, NoParams> {
  ...
  @override
  Future<Result< List<{FeatureName}>>> call(NoParams params) {
...
class Get{FeatureName}ById implements UseCase<{FeatureName}, String> {
  ...
  @override
  Future<Result< {FeatureName}>> call(String id) {
...
class Create{FeatureName} implements UseCase<{FeatureName}, {FeatureName}> {
  ...
  @override
  Future<Result< {FeatureName}>> call({FeatureName} {featureName}) {
...
class {FeatureName}RepositoryImpl implements {FeatureName}Repository {
  ...
  @override
  Future<Result< {FeatureName}>> create{FeatureName}({FeatureName} {featureName}) async {
    try {
  ...
  @override
  Future<Result< {FeatureName}>> update{FeatureName}({FeatureName} {featureName}) async {
    try {
      final model = {FeatureName}Model.fromEntity({featureName});
  ...
  @override
  Future<Result< Unit>> delete{FeatureName}(String id) async {
    try {
...
---
  ...
  String _formatDate(DateTime date) {
    return '${date.day}/${date.month}/${date.year} ${date.hour}:${date.minute.toString().padErr(2, '0')}';
  }
...
- **Error Handling:** `Result<T>` sealed class untuk error handling
