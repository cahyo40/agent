...
---
# Workflow: Offline Storage (GetStorage + Hive + SecureStorage)

// turbo-all

## Overview

Implementasi offline-first storage: GetStorage untuk simple cache,
Hive untuk complex data, dan flutter_secure_storage untuk
sensitive data. Termasuk connectivity monitoring.


## Prerequisites

- Project setup dari `01_project_setup.md` selesai
- Backend integration selesai (05/06/07)


## Agent Behavior

- **Gunakan GetStorage** untuk simple key-value (settings, flags).
- **Gunakan Hive** untuk complex objects (cache, offline data).
- **Encrypt sensitive data** — gunakan flutter_secure_storage.
- **Implement TTL** — cache expiry untuk freshness.


## Recommended Skills

- `senior-flutter-developer` — Offline patterns
- `senior-database-engineer-sql` — Local storage
...
class ProductRepositoryImpl implements ProductRepository {
  ...
  Result< List<Product>> _fromCache() {
    ...
    return const Err(CacheFailure('No cached data available'));
  }
}
```
