...
**Description:** Template untuk presentation layer (controller, screens dengan YoUI, widgets).
...
2. List screen dengan YoUI widgets dan semua states
3. Detail screen
4. Shimmer loading widget
5. List item widget dengan YoCard
6. Form widget
...
class {FeatureName}Controller extends _${FeatureName}Controller {
  ...
  Future<void> create{FeatureName}({required String name}) async {
    ...
    final result = await repository.create{FeatureName}(
      {FeatureName}(id: '', name: name, createdAt: DateTime.now()),
    );
...
### 5. Complete Screen Template with YoUI

**Description:** Template screen lengkap dengan semua states dan YoUI widgets.
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => ref
                .read({featureName}ControllerProvider.notifier)
                .refresh(),
          ),
        ],
      ),
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return YoCard(
                  ...
                  child: {FeatureName}ListItem(
                    {featureName}: {featureName},
                    onTap: () => context.push(
                      '/{featureName}s/${{{featureName}.id}',
                    ),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ref.read({featureName}ControllerProvider.notifier)
