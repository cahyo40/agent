...
Untuk menghindari UI freeze saat mem-parsing JSON payload besar (>1MB), gunakan `Isolate.run()` dari Dart 3. Ini memindahkan proses decoding ke background thread.
...
import 'dart:isolate';
  ...
  Future<Result<List<Product>>> getMassiveProducts() async {
    try {
      ...
      return ResultFailure(ServerFailure(message: 'Failed to parse massive products'));
    }
  }
```
