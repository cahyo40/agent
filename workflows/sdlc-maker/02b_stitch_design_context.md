...
- Use `stitch-design-md` skill to create the `stitch-design-context.md` file
- Use `stitch-enhance-prompt` skill to enhance each screen prompt
- This workflow runs AFTER `02_ui_ux_design.md` is complete
- Generate exactly one prompt per unique screen defined in the wireframes
- Prompts must be specific enough that Stitch AI can generate accurate UIs without guessing
- Save all output to `sdlc/02-ui-ux-design/stitch/`
